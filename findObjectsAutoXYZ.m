function objectsNext=findObjectsAutoXYZ(prev,next,objectsPrev,objectSize,searchSize,trackMethod)
plotting = true;
...
for o=1:N_objects
    ...
    inds_image =  arrayfun(@(i) max(floor(object_bounds(1,i)),1):min(ceil(object_bounds(2,i)),sz(i)),1:size(object_bounds,2),'UniformOutput',0);
    
    objectImage=prev(inds_image{:});
    
    if ndims(objectImage)==2
        objectImage=reshape(objectImage,[size(objectImage) 1])
    end
    ...
    inds_search =  arrayfun(@(i) max(floor(search_bounds(1,i)),1):min(ceil(search_bounds(2,i)),sz(i)),1:size(search_bounds,2),'UniformOutput',0);
    
    
    searchZone1=prev(inds_search{:});
    searchZone2=next(inds_search{:});
...
% (cell2mat(m2)-cell2mat(m1))
    objectsNext(o,:) = objectsPrev(o,:) + (cell2mat(m2)-cell2mat(m1))  ;
